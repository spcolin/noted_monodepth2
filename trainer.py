...
class Trainer:
    def __init__(self, options):
        ...
        # for the transformation between camera coordinate and pixel coordinate
    ...
    def generate_images_pred(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            for i, frame_id in enumerate(self.opt.frame_ids[1:]):
                ...
                # WHEN WILL self.backproject_depth AND self.project_3d OF OTHER SIZE USED???????????????????????
                cam_points = self.backproject_depth[source_scale](
                    depth, inputs[("inv_K", source_scale)])
                pix_coords = self.project_3d[source_scale](
                    cam_points, inputs[("K", source_scale)], T)

                # sample grids are all the same size,B*192*640*2
                outputs[("sample", frame_id, scale)] = pix_coords

                # grid_sample outputs are all the same size,B*3*192*640
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            disp = outputs[("disp", scale)]
            # color used for computing smooth
            color = inputs[("color", 0, scale)]
            target = inputs[("color", 0, source_scale)] #B*3*h*w


            for frame_id in self.opt.frame_ids[1:]:
                pred = outputs[("color", frame_id, scale)]
                reprojection_losses.append(self.compute_reprojection_loss(pred, target))    #[B*1*h*w],list

            reprojection_losses = torch.cat(reprojection_losses, 1) #B*source_frame_num*h*w

            if not self.opt.disable_automasking:
                # identity_reprojection_lossed compute the difference between unwarpped image and target image
            ...
            if not self.opt.disable_automasking:
                # DO NOT KNOW WHY ADD RANDN???????????????????????????????????????????????
                # add random numbers to break ties
                identity_reprojection_loss += torch.randn(
                    identity_reprojection_loss.shape).cuda() * 0.00001

                # put identity_reprojection_loss and reprojection_loss together for computing min value
